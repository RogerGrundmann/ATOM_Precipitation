...
| `ATM_RAD_TOPO` | **DEFAULT ON SINCE 2026-09-24** (`rt600` vs `el2`, 600 from scratch: land-only; land/ocean precip ratio 0.614 -> 0.769 against NASA 0.741, 35-65 deg +11.6 %, global +5.5 %, OLR -2 W/m2, velocities identical; `ATM_RAD_TOPO=0` restores; byte check both ways, 13 of 14, banner only; `ATM_RH_MIN_PTOP` re-sweep running because of the +5.5 %). Was: **NEW HERE, AND STILL DEFAULT OFF** — this tree's defect, not ATHAD's. The two "flipped on" claims below are WRONG, corrected 2026-09-02: `MultiLayerRadiation.h:293` is `e && atoi(e) != 0`, and every run's own `[RUN CONFIG]` banner prints `RAD_TOPO=0*` | inapplicable — no topography |
