...
def draw_precipitation_map(time=0, data_dir='./', output_dir='/tmp/atom/', topo_suffix='smooth'):
    gmt_cmd = 'gmt' 
    all_data = np.genfromtxt(data_dir + '/{0}Ma_{1}_PlotData_Atm.xyz'.format(time, topo_suffix), skip_header=1)
    data = all_data[:,8]
    ...
    cs = m.imshow(img_data,alpha=0.5, vmin=0, vmax=4000, cmap='jet')
...
def draw_nasa_precipitation_and_diff_map(time=0, data_dir='./', output_dir='/tmp/atom/', topo_suffix='smooth'):
    ...
    data = np.genfromtxt(data_dir + '/{0}Ma_{1}_PlotData_Atm.xyz'.format(time, topo_suffix), skip_header=1)        
