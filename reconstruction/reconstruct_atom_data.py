...
# ── Module-level caches (populated on first use, reused across calls) ─────────
_coastline_cache  = {}          # topo filename → polygon_features list
_land_mask_cache  = {}          # topo filename → bool array (65 341 points)
_rotation_cache   = {}          # reconstruction_dir → (static_polygons, rot_files, rot_model)

# Precomputed pygplates property names (cheap objects, but avoids repeated
# string hashing inside tight loops)
_PROP_DEPTH = pygplates.PropertyName.create_gpml('subductionZoneDepth')
_PROP_ORDER = pygplates.PropertyName.create_gpml('subductionZoneSystemOrder')


def _classify_points_land_ocean(points_lonlat, coastline_polygons):
    """
    Returns a boolean array: True = on land, False = ocean.
    Uses an R-tree spatial index; batches all queries with query_bulk
    (Shapely 1.8+) to avoid 65 K individual Python tree.query() calls.
    """
    n = len(points_lonlat)
    on_land = np.zeros(n, dtype=bool)

    shapely_polys = []
    for f in coastline_polygons:
        coords = f.get_geometry().to_lat_lon_list()
    ...
    prepped = [prep(p) for p in shapely_polys]

    # Build all Point objects in one pass
    pts = [Point(float(lon), float(lat)) for lon, lat in points_lonlat]

    try:
        # query_bulk (Shapely 1.8+): returns (point_indices, poly_indices)
        # for all bounding-box overlaps — one C-level call instead of 65 K
        pt_idxs, poly_idxs = tree.query_bulk(pts)
        for pi, poly_i in zip(pt_idxs, poly_idxs):
            if not on_land[pi] and prepped[poly_i].contains(pts[pi]):
                on_land[pi] = True
...
def _land_mask_for_topo(topo_filename, coastline_polygons):
    """
    Returns (and caches) the full 181×361 land mask for a given topo file.
    Temperature and precipitation runs share the same from/to-time polygons,
    so the mask is computed only once per unique topo file per process.
    """
    if topo_filename not in _land_mask_cache:
        lat = np.linspace(90, -90, 181)
        lon = np.linspace(-180, 180, 361)
        lon_2d, lat_2d = np.meshgrid(lon, lat)
        grid_lonlat = np.column_stack((lon_2d.ravel(), lat_2d.ravel()))
        _land_mask_cache[topo_filename] = _classify_points_land_ocean(
            grid_lonlat, coastline_polygons)
    return _land_mask_cache[topo_filename]
...
def interp_grid_gmt(a):
    ...
    np.savetxt(input_xyz, data, fmt='%f %f %f')
...
def gmt_filter(a, zmin=None, zmax=None):
    ...
    # Bulk write replaces Python loop
    np.savetxt(input_xyz, data_array, fmt='%f %f %f')
    ...
    return np.genfromtxt(output_xyz)[:, 2]



def get_coastline_polygons_from_topography(filename):
    if filename in _coastline_cache:
        return _coastline_cache[filename]
    ...
    _coastline_cache[filename] = polygon_features
...
def reconstruct_grid(from_time, input_grid, to_time, output_grid, reconstruction_dir=script_dir):
    ...
    topo = topo[:, 2]
    topo_mask = np.where(topo > 0, True, False)
    remove_data_nearby_coastline_fast(data, topo_mask, 5)

    data = add_lon_lat_to_gmt_data(data)

    # Load rotation resources once per reconstruction_dir; reuse across calls
    if reconstruction_dir not in _rotation_cache:
        rot_files = [reconstruction_dir + '/data/Rotations/Global_EarthByte_230-0Ma_GK07_AREPS.rot']
        _rotation_cache[reconstruction_dir] = (
            pygplates.FeatureCollection(
                reconstruction_dir + '/data/Muller_etal_AREPS_2016_StaticPolygons.gpmlz'),
            rot_files,
            pygplates.RotationModel(rot_files))
    static_polygons, rotation_files, rotation_model = _rotation_cache[reconstruction_dir]

    topo_from = TOPO_DIR + '/{}Ma_smooth.xyz'.format(from_time)
    topo_to   = TOPO_DIR + '/{}Ma_smooth.xyz'.format(to_time)
    coastline_polygons_from_time = get_coastline_polygons_from_topography(topo_from)
    coastline_polygons_to_time   = get_coastline_polygons_from_topography(topo_to)

    # Land mask for from_time: cached so temperature + precipitation share it
    data = data.reshape((181 * 361, 3))
    on_land = _land_mask_for_topo(topo_from, coastline_polygons_from_time)

    # Split into land features and ocean points using numpy indexing
    land_indices  = np.where(on_land)[0]
    ocean_mask    = ~on_land & ~np.isnan(data[:, 2])
    points_in_ocean = data[ocean_mask].tolist()
    ...
    for idx in land_indices:
        ...
        f.set_double(_PROP_DEPTH, point[2])
        f.set_integer(_PROP_ORDER, int(idx))
        points_on_land.append(f)

    # Partition and rotate land points (unchanged logic, cleaner grouping)
    partitioned_features, _ = pygplates.partition_into_plates(
        static_polygons, rotation_files, points_on_land,
        properties_to_copy=[pygplates.PartitionProperty.reconstruction_plate_id],
        reconstruction_time=from_time,
        partition_return=pygplates.PartitionReturn.separate_partitioned_and_unpartitioned)

    # Group by plate ID, apply single rotation per plate
    ...
    for key, group in groupby(fs, lambda x: x.get_reconstruction_plate_id()):
        ...
        for f in group:
            ...
            v = f.get_double(_PROP_DEPTH)
            new_points_on_land.append([ll[1], ll[0], v])

    # Interpolate ocean, then classify against to_time coastlines
    grid_data = interp_grid_gmt(points_in_ocean)
    grid_data = grid_data.reshape((181, 361))

    data = add_lon_lat_to_gmt_data(grid_data)
    data = data.reshape((181 * 361, 3))

    # Land mask for to_time: cached across calls with the same to_time
    on_land_to = _land_mask_for_topo(topo_to, coastline_polygons_to_time)
    points_in_ocean_to_time = data[~on_land_to].tolist()

    # Filter reconstructed land points to those inside to_time polygons
    if new_points_on_land:
        land_arr = np.array(new_points_on_land)
        land_on = _classify_points_land_ocean(land_arr[:, :2], coastline_polygons_to_time)
        new_grid_data = points_in_ocean_to_time + [new_points_on_land[i]
        for i in range(len(new_points_on_land))
        if land_on[i]]
    else:
        new_grid_data = points_in_ocean_to_time

    grid_data = interp_grid_gmt(new_grid_data)
    grid_data = grid_data.reshape((181, 361))
...
def reconstruct_velocity_grid(from_time, input_grid, to_time, output_grid, reconstruction_dir=script_dir):
    ...
    # Land masks: cached so wind-v and wind-w share the same mask computation
    data = data.reshape((181 * 361, 3))
    on_land = _land_mask_for_topo(topo_from, coastline_polygons_from_time)
    ocean_mask = ~on_land & ~np.isnan(data[:, 2])
    points_in_ocean = data[ocean_mask].tolist()
    ...
    on_land_to = _land_mask_for_topo(topo_to, coastline_polygons_to_time)
    new_grid_data = data[:, 2].copy()
    new_grid_data[on_land_to] = 0.0

    grid_data = new_grid_data.reshape((181, 361))
...
def run_reconstruction(time_0, time_1, suffix, atm_or_hyd):             #  atm_or_hyd: 'atm' or 'hyd'
    if atm_or_hyd == 'atm':
        reconstruct_temperature(time_0, time_1, suffix)
        reconstruct_precipitation(time_0, time_1, suffix)
#        reconstruct_wind_v(time_0, time_1, suffix)
#        reconstruct_wind_w(time_0, time_1, suffix)
    else:
        reconstruct_salinity(time_0, time_1, suffix)
...
def main():                                                             #  this python program is called in FileIO_Atm.cpp and executed by int ret = system(cmd_str.c_str());
    try:
        ...
        print()
        print('main in reconstruct_atom_data.py, actual timestep = time_1 compares to  Ma = ', time_1)
        print()

        run_reconstruction(time_0, time_1, BATHYMETRY_SUFFIX, atm_or_hyd)
